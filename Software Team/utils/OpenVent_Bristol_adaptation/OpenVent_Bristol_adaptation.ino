...
int target_BPM = 30;            // defult 20 BPM
...
void setMotor1Speed(int M1speed)
{
  ...
  constrain(M1speed, -400, 400); // Constrain to Max PWM dutycycle
...
int getPressure(){
  ...
  return sensorCluster.getRelativePressureC();
}
/*
...
// **** check home made variable orifice sensor
int getFlowRate ()
{
  int num = 20;
  float diffPress = 0;
  for (int i = 0; i < num; i++) {
    diffPress += sensorCluster.getDifferentialPressure();
  }
  diffPress /= num;

  return sensorCluster.calcFlow(diffPress)/1000; // flow in ml/min;
}
