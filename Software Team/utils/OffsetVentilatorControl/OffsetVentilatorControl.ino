/*
  OffsetVentilatorControl_V2
  Is highly based and insbired by
...
SensorClusterHandler sensorCluster(PIN_CS_1, PIN_CS_2, PIN_CS_3); // SPI PINS & CS1-3
MotorControl motorControl(PIN_MOTOR_A, PIN_MOTOR_B);
...
// Pressure
#define TARGET_PRESSURE_CMH2O 35
#define MIN_PRESSURE_CMH2O 15                // set higher than the lowest PEEP for failsafe
#define MAX_PRESSURE_CMH2O 45

// Volume
#define MIN_TIDALVOLUME_ML_MIN 150                // set higher than the lowest PEEP for failsafe
#define MAX_TIDALVOLUME_ML_MIN 800

BreathCyclusHandler breathCyclusHandler(BPM, IEVAL);
PressureHandler pressureHandler(TARGET_PRESSURE_CMH2O, MIN_PRESSURE_CMH2O, MAX_PRESSURE_CMH2O);
FlowHandler flowHandler(MIN_TIDALVOLUME_ML_MIN, MAX_TIDALVOLUME_ML_MIN);
...
double Kp = 6, Ki = 5, Kd = 0.04;   // 35cmH2O default
PID myPID(&Input, &Output, &Setpoint, Kp, Ki, Kd, DIRECT);
unsigned long serialTime; //this will help us know when to talk with processing


// setting for moving the arm back in exhale
int revTimeSetting = 450;       // time it takes to reverse in ms
const int revSpeed = 400;             // max speed of reverse stroke
const int maxFwdSpeed = -400;         // max speed of motor/acceloration limit


// alarm states
bool alarmMotorStop = 0;

unsigned long breathCyclusStartTimeStamp = 0;
unsigned long revTimeStamp = 0;
unsigned long motorSetTime = 0;
...
void setup()
{
  ...
  sensorCluster.begin();
  delay(1000);
  sensorCluster.readSensor(50, true);
  motorControl.begin();
  ...
  Input = MIN_PRESSURE_CMH2O;
  Setpoint = pressureHandler.getTargetPressure();       // flowrate target
...
/*
  Serial.print("breath period: ");
  Serial.println(breathCyclusHandler.getTargetBreathPeriod());
  Serial.print("inhale period: ");
  Serial.println( breathCyclusHandler.getTargetInhalePeriod());
  */
  ...
  /*
    ...
    digitalWrite(BUZZER_PIN, HIGH);    // test buzzer
    delay(50);
    ...
    setDisplayHeadings();
  */
  breathCyclusState = EXHALE_PAUSE;
...
/*
  ...
  else if (leftButtonState == 1 && millis() - leftTimer > offButtonDebounceTime && breathCyclusState == exhalePause)
...
void loop()
{
  ...
  // read sensors once;
  sensorCluster.readSensor(20);
  pressureHandler.update( sensorCluster.getRelativePressureC() );
  flowHandler.update( sensorCluster.getFlow() );
  breathCyclusHandler.update( millis() );

  // check for Alarms

  boolean alarm = false;
  alarm = alarm || pressureHandler.checkMinMax();
  alarm = alarm || flowHandler.checkTidalMinMax();
  //alarm = alarm || motor.checkStuck();
  if (alarm) {
    soundBuzzer(1);
    //breathCyclusState = STOP;
  }

  switch (breathCyclusState) {
    case MOVE_TO_BAG: {
        motorControl.setSpeed(-300);
        if (pressureHandler.getFilteredPressure() != pressureHandler.getPressure()) {
          motorSetTime = millis() - breathCyclusStartTimeStamp;
          breathCyclusState = INHALE;
        }
      } break;

    case INHALE: {
        doPressureControl(pressureHandler.getPressure());
        if (breathCyclusHandler.isExhale()) {
          revTimeStamp = millis();
          accelTime = millis();               // log time in ms for acceloration

          breathCyclusState = MOVE_BACK;
        }
      } break;

    case MOVE_BACK: {
        // reverse motor
        //revTimeSetting = map(flowHandler.getTidalVolume(), 200, 800, 216, 315);
        ...
        if (revAccelSpeed < revSpeed)            // acceloration
        {
          ...
          {
            revAccelSpeed += 40;                 // - is forward direction
            accelTime = millis();
          }
        }
        //revAccelSpeed = 400;
        motorControl.setSpeed(revAccelSpeed);

        if (millis() - revTimeStamp > revTimeSetting) {
          breathCyclusState = EXHALE_PAUSE;
        }
      } break;
    ...
    case EXHALE_PAUSE: {
        ...
        if (breathCyclusHandler.isFinished()) {
          breathCyclusHandler.startCycle(millis());
          breathCyclusState = MOVE_TO_BAG;
        }
      } break;
  ...
  int criticalLine = pressureHandler.getTargetPressure() - (2 + (pressureHandler.getTargetPressure() * 0.04));
  //serialPlot(pressureHandler.getTargetPressure(), pressureHandler.getPressure(), pressureHandler.getFilteredPressure(), motorControl.getSpeed() / 10, criticalLine);

  Serial.print(pressureHandler.getTargetPressure());
  Serial.print("  ");
  Serial.print(criticalLine);
  Serial.print("  ");
  Serial.print(pressureHandler.getPressure());
  Serial.print("  ");
  Serial.print(pressureHandler.getFilteredPressure());
  Serial.print("  ");
  Serial.print(motorControl.getSpeed() / 10);
  Serial.print("  ");
  Serial.print(breathCyclusState * 10);
  Serial.print("  ");
  Serial.print(flowHandler.getTidalVolume()/10);
  Serial.print("  ");
  Serial.println("uT");
...
void serialPlot (int val1, int val2, int val3, int val4, int val5)
{
...
// *********** Inhale ***********
int doPressureControl(int pressure)
{
  ...
  */
  Setpoint = pressureHandler.getTargetPressure();                                      // pressure target
  ...
  motorControl.setSpeed(maxFwdSpeed);
...
void soundBuzzer(bool buzzerSetting)          // sounds the buzzer during an alarm state
{
  static unsigned long volAlarmTimer;        // count time

  if (buzzerSetting == 1)
  {
    digitalWrite(BUZZER_PIN, HIGH);           // sonund buzzer
...
/*
  ...
  if (breathCyclusState == INHALE)        // if during inhale
  {
